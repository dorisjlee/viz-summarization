...
function showfilter(cell){
    ...
    var idx = cell.id.slice(1);

    title = Dic[idx][Dic[idx].length-1]["filter"]
        if (idx==0){
                title = "root";
            }
            else{
    ...
    if(tableChecked[idx]==false){
        document.getElementById('selected').innerHTML+=  idx +":  "+ title + "<br />";
    }
    tableChecked[idx] = true;
}
