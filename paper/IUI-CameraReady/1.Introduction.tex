...
(which grows exponentially in number of attributes) presents a major bottleneck \change{during exploration}.
...
\caption{Example data subset lattice from the 2016 US election dataset illustrating the drill-down fallacy along the purple path as opposed to the informative orange path.\agp{please avoid "Black" in favor of African-American}}
...
{\bf {\em drill-down fallacy}}---a new family of errors
in reasoning we identify---where incomplete insights result from potentially confounding factors not explored along a drill-down path}. As shown in Figure~\ref{fig:elections_example}, an analyst can arrive at the \texttt{Black Female} visualization \change{via} the purple or the orange drill-down path. An analyst who followed the purple path 
may be surprised at how drastically the \texttt{Black Female} 
voting behavior differs from that of \texttt{Female}. 
However, this behavior is not surprising if 
the analyst had gone down the orange path that we saw earlier, where the proper reference (i.e., the distribution for \texttt{Black}) 
explains the vote distribution for \texttt{Black Female}. 
In other words, even though the vote distribution for \texttt{Black Female} is very different from that of \texttt{Female}, 
the phenomenon can be explained by a more general ``root cause'' attributed to the voting behavior for the \texttt{Black} 
...
\change{In order to not fall prey to such drill-down fallacies---``surprising'' but not significant 
local differences in trend during drill-down 
(\texttt{Female} $\rightarrow$ \texttt{Black Female})---it is important to preserve the proper parent reference (\texttt{Black})
to contextualize the behavior of the visualization of interest (\texttt{Black Female}).}
One approach to avoid this fallacy 
is to \change{exhaustively} 
explore all potential drill-down paths. 
Unfortunately, this approach does not
scale.

\par
\change{
While there have been a number of statistical
reasoning fallacies that have been identified
in the literature, including Simpson's Paradox~\cite{Guo2017}, XXX,
...
systems~\cite{vida,vizrec} that
assist or accelerate the process of visual
data exploration~\cite{Vartak2015,zenvisage,Wongsuphasawat2016,profiler,smart-drill-down,data-polygamy,visdom}, 
none of these systems have provided
a conclusive solution to the problem of aiding or
accelerating drill-downs
to explore data subsets, 
while avoiding drill-down fallacies.
We discuss this and other 
related work in more detail in Section~\ref{sec:related}.}
 ...
 insights in \change{the} dataset). 
 ...
 network of {\bf \em interesting} and {\bf \em informative} visualizations 
 that {\bf \em collectively} convey the key 
 insights in a dataset}. 
 Our user study results demonstrate that 
 \system can help analysts gain a better understanding
 of the dataset and guide them toward meaningful 
...
\change{\textit{truly interesting} 
