...
that leads to the highest gain in utility from the frontier at every step. Starting from a pruned lattice comprising only connections to informative parents (left) and three nodes in the existing solution (blue), we select the node with the highest utility gain (yellow) amongst the frontier nodes (green). On the right, the newly added node results in an updated frontier and the node 
...
In this section, we present our system, \system, by first providing a high-level overview of the underlying algorithm, 
...
\change{
  ...
  (i.e., filters) to construct the lattice upfront. We retain
  only the edges that correspond to informative parents: this
  helps simplify the presentation somewhat.  
...
except the \change{selected visualization is set as the 
