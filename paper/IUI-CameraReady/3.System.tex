...
Throughout, the algorithm maintains a list of {\em frontier} nodes---nodes
...
in $S'$. \achange{Given a pre-materialized lattice where only edge connections to informative parents are retained, any frontier node is guaranteed to have an informative parent in the the existing solution and can be added to $S$ without violating the informativeness principle.}
...
\begin{algorithm}
  ...
  \While{$|S'| < k$}
      \State frontier $\gets$ getFrontier($S'$, \achange{$\mathcal{L}$})
