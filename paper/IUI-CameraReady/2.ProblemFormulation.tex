...
\change{\subsection{Design Principles for Informative Exploration}}
...
the analysts to form a accurate mental model
...
in this manner is not new---we leverage prior work for this~\cite{Vartak2015,Siddiqui2017,Macke2018,Ding2016}.
...
We define a parent to be  
informative (denoted $V_i^*$) 
if its distance from the child 
falls within a threshold $\theta\%$ of the 
...
is only defined in the presence of informative parent references.
...
parent, if present\footnote{If multiple informative parents, $V_i^{*, \theta}$, are present for a given visualization, $V_i$, then $U(V_i)$ is defined in terms of the most informative parent that is present.}. 
...
\par \textsc{Problem.} \textit{Given a dataset and user-provided X, Y attributes, select a subset $S, |S| = k$ visualizations from the lattice of data subsets $\mathcal{L}$, such that $U(S)$ is maximized.}
\\
