...
(we will formalize our notion of deviation subsequently) 
between
a parent and a child to be significant. 
For example, they may be surprised by the fact
that the \texttt{Female} and \texttt{Black Female}
visualizations are very different from each other,
and find this to be a novel insight.
However, this deviation 
is because \texttt{Female} is not
the {\em proper} parent or reference for
\texttt{Black Female}.
Here, a different parent, \texttt{Black},
is the proper reference of \texttt{Black Female} 
because it is one that exhibits the least deviation relative 
to \texttt{Black Female}.
In this case, we find 
that the \texttt{Black Female} visualization
is not really all that surprising given the \texttt{Black}
visualization. 
We refer to this phenomenon of being deceived by
a local difference or deviation relative to an improper 
reference as an instance of the {\em drill-down fallacy}.
The way to avoid such fallacies is to ensure that the proper parent
is present for each visualization so that analysts
do not draw incorrect conclusions. 
While this fallacy is applicable to any 
chart type that can be described as a 
probability distribution over data 
(e.g., bar charts, pie charts, heatmaps), 
