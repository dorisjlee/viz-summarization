...
\achange{\subsection{The ``3S'' Design Principles for Insightful Exploration}}
Our goal is to help analysts discover
the key insights in a dataset,
...
the \achange{analyst to form an} accurate mental model
...
in this manner is not \achange{a novel concept}---we leverage prior work for this~\cite{Vartak2015,Siddiqui2017,Macke2018,Ding2016}.
...
from that of its \achange{informative} parents,
...
is only defined \achange{with respect to} informative parent references.
...
parent, if present\footnote{If multiple informative parents, \achange{$V_i^*$}, are present for a given visualization, $V_i$, then $U(V_i)$ is defined in terms of the most informative parent present.}.%$V_i^{*, \theta}$
To incorporate the effect of subpopulation size into our objective function, we multiply the distance $D(V_i, V_i^*)$ between an informative parent $V_i^*$ and a child visualization $V_i$ by the ratio of their sizes.
...
\change{
We can't possibly display all of the visualizations
...
where we only return a subset $S$ of all visualizations of size $|S| = k$.
We define the utility of $S$:
...
\par \textsc{Problem.} \textit{Given a dataset and user-provided X, Y attributes, select a subset $S, \achange{consisting of} |S| = k$ visualizations from the lattice of data subsets $\mathcal{L}$, such that $U(S)$ is maximized.}
\\
...
\agp{This doesn't actually show that our problem is hard. We need to reduce max-weighted subgraph to our problem, not the other way around. I am leaving a pointer here
to think about it later.}
\achange{Next,} we design several approximate algorithms to solve this problem efficiently.
}
