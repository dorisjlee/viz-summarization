...
In this section, we first describe our data model by reporting our data, visualization and query setup, and the underlying hierarchy of data subsets. We then introduce our user model to traverse the hierarchy and justify its efficacy through a user study. Finally, we present our problem of finding informative and interesting visualizations from the hierarchy.
...
Now, we discuss the set-theory-based \emph{containment} relationships for data subsets, and based on the relationships define our hierarchy.
