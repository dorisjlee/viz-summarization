...
\begin{document}
\title{\system : Exploring Insights with Informative Drill-Downs}
% \author{\plainauthor}
\begin{abstract}
...
As datasets continues to grow in size and complexity, exploring a multi-dimensional dataset remains a challenging exercise for analysts. A common operation of this exploration is drill-down–-accessing more specific data by adding filters. While widely used, in absence of careful attention towards confounding factors, drill-down could lead to inductive fallacies. Specifically, an analyst may end up being \lq\lq deceived\rq\rq\ into thinking that a deviation in trend is attributable to a local change, when in fact it is a global phenomenon; we call this the drill-down fallacy. A naive solution to prevent drill-down fallacy is to explore all sub-paths of a drill-down path, which quickly becomes infeasible as an analyst explores more and more paths. In this paper, we present \system, a visual data exploration tool that leads to the key meaningful insights in a dataset avoiding the drill-down fallacies. Our user study result shows that our tool can guide an analyst towards meaningful insights for a variety of tasks.
\end{abstract}
