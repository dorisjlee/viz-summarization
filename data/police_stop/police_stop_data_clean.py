# Collected all datasets with complete information (indicated on the stanford website)
# eliminated IL and WA data which had all is_arrested as NaN, and VT eliminated because no stop duration

import pandas as pd
from glob import glob
from sklearn import preprocessing
df_all=[]
for fname in ["IL-clean.csv","WA-clean.csv"]:#glob("*-clean.csv"):
	...
	'search_type_raw', 'search_type','police_department','officer_id',"violation","violation_raw","driver_age","driver_age_raw","driver_race_raw"],axis=1)
		...
		# Illinois has no officer_id
		df = df.drop(["stop_date","stop_time","location_raw","county_name",'county_fips','stop_hr','fine_grained_location',\
	'search_type_raw', 'search_type','police_department',"violation","violation_raw","driver_age","driver_age_raw","driver_race_raw"],axis=1)
	cols = ['id', 'state','driver_gender', 'driver_race', 'search_conducted', 'contraband_found', 'stop_outcome', 'is_arrested', 'stop_duration', 'stop_time_of_day', 'driver_age_category']
	df = df[cols]
	df =df.dropna()
...
df.to_csv("all_state_police_stop.csv")
#Upload to database:
